...
def _yahoo_quote(data: dict[str, Any], symbol: str) -> dict[str, Any]:
    ...
    return {'symbol': symbol, 'price': result.get('regularMarketPrice'), 'change': result.get('regularMarketChange'), 'change_percent': result.get('regularMarketChangePercent'), 'high': result.get('regularMarketDayHigh'), 'low': result.get('regularMarketDayLow'), 'open': result.get('regularMarketOpen'), 'previous_close': result.get('regularMarketPreviousClose'), 'timestamp': datetime.now(timezone.utc).isoformat(), 'source': 'yahoo_finance'}


async def _yahoo_chart(symbol: str, days: int) -> list[dict[str, Any]]:
    period2 = int(datetime.now(timezone.utc).timestamp()); period1 = int((datetime.now(timezone.utc) - timedelta(days=days)).timestamp())
    url = f'https://query1.finance.yahoo.com/v8/finance/chart/{symbol}'
    async with httpx.AsyncClient(timeout=settings.REQUEST_TIMEOUT, headers={'User-Agent': 'Mozilla/5.0'}) as client:
        r = await client.get(url, params={'period1': period1, 'period2': period2, 'interval': '1d', 'events': 'history'}); r.raise_for_status(); data = r.json()
    result = (data.get('chart', {}).get('result') or [{}])[0]
    meta = result.get('meta', {})
    timestamps = result.get('timestamp') or []
    quote = (result.get('indicators', {}).get('quote') or [{}])[0]
    return [{'timestamp': t, 'open': o, 'high': h, 'low': l, 'close': c, 'volume': v} for t,o,h,l,c,v in zip(timestamps, quote.get('open',[]), quote.get('high',[]), quote.get('low',[]), quote.get('close',[]), quote.get('volume',[])) if c is not None][-days:]
...
async def get_quote(symbol: str) -> dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.REQUEST_TIMEOUT, headers={'User-Agent':'Mozilla/5.0'}) as client:
            r=await client.get(f'https://query1.finance.yahoo.com/v7/finance/quote', params={'symbols':symbol}); r.raise_for_status(); data=r.json()
        result=(data.get('quoteResponse',{}).get('result') or [])
        if result: return _yahoo_quote(data,symbol)
    except (httpx.HTTPError, ValueError, KeyError): pass
    if not settings.FINNHUB_API_KEY: raise MarketDataError(f'No market data available for {symbol}')
    try:
...
async def get_history(symbol: str, days: int = 260) -> list[dict[str, Any]]:
    symbol=symbol.strip().upper(); days=max(60,min(days,1000))
    try:
    ...
    now=int(datetime.now(timezone.utc).timestamp()); start=now-days*86400
    try:
        async with httpx.AsyncClient(timeout=settings.REQUEST_TIMEOUT) as client:
