...
async def _finnhub_quote(symbol: str) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=min(settings.REQUEST_TIMEOUT, 12.0)) as client:
        r = await client.get(
...
async def _yahoo_chart(symbol: str, days: int) -> list[dict[str, Any]]:
    ...
    async with httpx.AsyncClient(timeout=min(settings.REQUEST_TIMEOUT, 12.0), headers={"User-Agent": "Mozilla/5.0"}) as client:
    ...
    return [
        ...
        for t, o, h, l, c, v in zip(
            timestamps, quote.get("open", []), quote.get("high", []), quote.get("low", []),
...
async def get_quote(symbol: str) -> dict[str, Any]:
    ...
    # Prefer the configured Finnhub key for the live quote. This avoids
    # Yahoo quote endpoint failures/rate limits on Render.
