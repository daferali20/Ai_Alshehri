...
async def get_current_subscription(user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if not sub:
        sub = UserSubscription(user_id=user.id, tier=SubscriptionTier.FREE.value, is_active=True); db.add(sub); db.commit(); db.refresh(sub)
...
async def stock_analysis(symbol: str, user: User = Depends(current_user)):
    try:
        ...
        technical = analyze_ohlcv(history) if history else None; liquidity = analyze_liquidity(history) if history else None; momentum = analyze_momentum(history) if history else None
        return {"quote": quote, "analysis": analyze_quote(quote), "technical": technical, "liquidity": liquidity, "momentum": momentum, "ranking": rank_stock(technical or {}, liquidity or {}, momentum or {}, quote)}
