...
@app.get("/api/v1/screener/us")
async def screener_us(user: User = Depends(current_user)):
    return {"status": "ok", "results": await screen_us_stocks(limit=20)}
...
async def get_current_subscription(user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if not sub:
        sub = UserSubscription(user_id=user.id, tier=SubscriptionTier.FREE.value, is_active=True)
...
async def register(request: RegisterRequest, db: Session = Depends(get_db)):
    if not request.terms_accepted:
        raise HTTPException(400, "يجب قبول الشروط والأحكام")
    ...
    db.add(user); db.flush(); db.add(UserSubscription(user_id=user.id, tier=SubscriptionTier.FREE.value, is_active=True)); db.commit()
...
async def upgrade_subscription(request: SubscriptionUpgradeRequest, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    sub = db.scalar(select(UserSubscription).where(UserSubscription.user_id == user.id)) or UserSubscription(user_id=user.id)
    sub.tier = request.tier; sub.is_active = True; sub.start_date = datetime.now(timezone.utc).replace(tzinfo=None); sub.end_date = sub.start_date + timedelta(days=365)
...
async def stock_analysis(symbol: str, user: User = Depends(current_user)):
    try:
        quote, history = await __import__("asyncio").gather(get_quote(symbol), get_history(symbol, 260))
        technical = analyze_ohlcv(history) if history else None
        liquidity = analyze_liquidity(history) if history else None
        momentum = analyze_momentum(history) if history else None
        ranking = rank_stock(technical or {}, liquidity or {}, momentum or {}, quote)
        return {"quote": quote, "analysis": analyze_quote(quote), "technical": technical, "liquidity": liquidity, "momentum": momentum, "ranking": ranking}
...
async def recommendations(request: RecommendationRequest, user: User = Depends(current_user), db: Session = Depends(get_db)):
    sub = db.scalar(select(UserSubscription).where(UserSubscription.user_id == user.id)); tier = sub.tier if sub else "free"
    max_symbols = next(x["features"]["maxSymbols"] for x in TIERS if x["id"] == tier)
    symbols = list(dict.fromkeys(s.strip().upper() for s in request.symbols if s.strip()))
    if max_symbols != -1 and len(symbols) > max_symbols:
        raise HTTPException(403, f"خطتك تسمح بتحليل {max_symbols} أسهم فقط")
    results = []
    for symbol in symbols:
        try:
            ...
            results.append({"symbol": symbol, "quote": quote, "technical": technical, "liquidity": liquidity, "momentum": momentum, "ranking": rank_stock(technical, liquidity, momentum, quote)})
        except MarketDataError as exc:
