...
const PricingTable: React.FC<PricingTableProps> = ({ 
  ...
  // تعريف جميع الميزات للجدول
  const featureRows = [
    { key: 'aiRecommendations', label: 'توصيات ذكاء اصطناعي', icon: '🤖' },
    { key: 'maxSymbols', label: 'عدد الأسهم', icon: '📊', custom: true },
    { key: 'updateInterval', label: 'تحديث البيانات', icon: '🔄', custom: true },
  ...
  const getDisplayValue = (tier: SubscriptionTier, featureKey: string) => {
    ...
    if (featureKey === 'maxSymbols') {
      return value === -1 ? 'غير محدود' : `${value}`;
    }
  ...
  const getColorForTier = (tierId: string) => {
    ...
    return colors[tierId as keyof typeof colors] || 'border-gray-600';
  };
  ...
  return (
    <div className="overflow-x-auto">
        ...
        <thead>
          <tr className="border-b border-gray-700">
            ...
            {tiers.map((tier) => (
              <th 
                key={tier.id} 
                className={`px-6 py-4 text-center text-sm font-bold text-white border-b-2 ${getColorForTier(tier.id)}`}
              >
                <div>{tier.name}</div>
                ...
                {currentTier === tier.id && (
                  <div className="text-xs text-green-500 font-normal mt-1">✓ الحالي</div>
                )}
              </th>
            ))}
          </tr>
                ...
                return (
                  <td key={`${tier.id}-${row.key}`} className="px-6 py-4 text-center">
                    {isBoolean ? (
                      ...
                      <span className="text-sm text-gray-300">{displayValue}</span>
                    )}
                    {row.key === 'autoExecution' && value && (
      ...
      {/* ملاحظة التنفيذ التلقائي */}
      <div className="px-6 py-4 bg-yellow-500/5 border-t border-yellow-500/30">
        <p className="text-xs text-yellow-500">
          * التنفيذ التلقائي للأوامر يتطلب اشتراك PREMIUM وإضافة مفاتيح API الخاصة بك مع موافقة قانونية صريحة
        </p>
      </div>
    </div>
  );
