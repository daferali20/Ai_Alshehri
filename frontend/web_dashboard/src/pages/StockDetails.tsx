  ...
  return (
    <section className="stock-details">
      <div className="stock-details__header">
        ...
        <div><h1>{symbol}</h1><span className="detail-muted">تحليل كمي وذكاء اصطناعي</span></div>
      </div>
      <article className="detail-card detail-card--hero"><StockChart history={data.history || []} /></article>
      <div className="stock-details__grid">
        <article className="detail-card detail-card--hero"><h3>التقييم النهائي</h3><div className="detail-value">{Number(r.score || 0).toFixed(1)} / 100</div><p>{r.label || 'لا يوجد تصنيف'}</p></article>
